...
void textureMapping(Mat* srcImages, int gridSize, int imgW, int imgH, int warpType, Mat*T, Mat H01, Mat* H, Mat *H01_matrices,Mat* c1para, double theta, Mat &dst, double ub1, double ub2, int zero_On)
{
	...
	for (int i = 0; i < NUMOFIMAGES; i++)
	{
		...
		/*	if (row < imgH / 2)
			...
			for (double col = 0; col < imgW; col = col + colStep)
			{
				...
				{
					//�����ǳ���H����
					double value = (double)(h00*X_temp + h01*Y_temp + h02) / (h20*X_temp + h21*Y_temp + h22);//����������˺ܴ��ƫ��,����Ϊmatlab��H�����õ���c1param�е�H�����H��H01��������Ϊ������H�е�H
					dataX[x] = value;
				...
				else if (u > ub1&&u <= ub2)
				{
					...
					else
					{
						...
						dataY[x] = (e0*u*u*u + e1*u*u + e2*u + e3)*v + f0*u*u + f1*u + f2;//����һ��ʼ��e3д����a3���������յ����ɵ�ͼ��������
					}
				}
				else
				{
	...
	for (int i = 0; i < NUMOFIMAGES; i++)
	{
		...
		//Mat temp = xy_mat[i]*tt;//matlab�������õ���apply_transform��������˼�������һ��
		Mat temp = applyTransform(xy_mat[i], tt);
		array2Matrix(temp.col(0), mask, mapX[i]);//����Ҳ�õ���array2Matrixע��ֵ����ȷ��
	...
	/*����û�м�matlab��scale down��һ������Ϊmatlab��������һ
	������Ϊ�˷�ֹ�ڴ����������ͨ����СͼƬ�ߴ����*/
	int tr_x = 5, tr_y = 5;
	...
	//mapping�������ͼ���mask����ÿһ��ͼ��������Ҫ����blending
	Mat dsts[2];
	Mat masks[2];
	...
	case 0:
		for (int i = 0; i < NUMOFIMAGES; i++)
		{
			...
			else
			{
				...
				{
					//locationsOfNonZero2Cols��һ����x���꣬�ڶ�����y����
				...
				//�������յ�ȫ��ͼ
				Mat weight_1 = d_mask1 + (1 - mask2).mul(matNot(mask2));//��������mat���������Ͳ�һ��
